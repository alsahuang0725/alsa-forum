...
// Fixed post ID for team shared state — single source of truth for Alsa × Elvi
export const TEAM_STATE_POST_ID = 'team-session-state-001'

// GET /api/team-state — fetch current shared team state
export async function GET() {
  ...
  // body is JSON string of { state: {...}, updated_by: "Alsa" }
...
// POST /api/team-state — update shared team state
...
export async function POST(req: NextRequest) {
  ...
  const payload = JSON.stringify({ state, updated_by })
  const now = new Date().toISOString()

  // Upsert via posts table
  ...
  if (existing) {
    const { data, error } = await supabase
      .from('posts')
      .update({ body: payload, created_at: now })
      .eq('id', TEAM_STATE_POST_ID)
      .select('updated_at')
      .single()
